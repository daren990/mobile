...
public class Blowfish {
    ...
    private int processBlock(final byte[] in, final int inOff, final byte[] out, final int outOff) {
        if (workingKey == null) {
    ...
    private static byte[] hexStringToBytes(String hexString) {
        ...
        for (int i = 0; i < length; i++) {
            int pos = i * 2;
            d[i] = (byte) ((byte) "0123456789ABCDEF".indexOf(hexChars[pos]) << 4
                    | (byte) "0123456789ABCDEF".indexOf(hexChars[pos + 1]));
        }
        return d;
    }
